...
func pb2goStmts(rootPkg *amino.Package, isRoot bool, imports *ast.GenDecl, scope *ast.Scope, goo ast.Expr, gooIsPtr bool, gooType *amino.TypeInfo, pbo ast.Expr, fopts amino.FieldOptions, options uint64) (b []ast.Stmt) {
	...
	case reflect.Int:
		b = append(b,
			_a(goor, "=", _call(_i("int"), maybeUnwrap(pbo))))
	case reflect.Int16:
		b = append(b,
			_a(goor, "=", _call(_i("int16"), maybeUnwrap(pbo))))
	case reflect.Int8:
		b = append(b,
			_a(goor, "=", _call(_i("int8"), maybeUnwrap(pbo))))
	case reflect.Uint:
		b = append(b,
			_a(goor, "=", _call(_i("uint"), maybeUnwrap(pbo))))
	case reflect.Uint16:
		b = append(b,
			_a(goor, "=", _call(_i("uint16"), maybeUnwrap(pbo))))
	case reflect.Uint8:
		b = append(b,
			_a(goor, "=", _call(_i("uint8"), maybeUnwrap(pbo))))
	...
	default:
		// General translation.
		b = append(b, _a(goor, "=", maybeUnwrap(pbo)))
	}
	return b
}
