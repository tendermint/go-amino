...
/*
	...
	On init, you should call NewMapper() for each interface type you want to
	support flexible de-serialization, and then RegisterImplementation() in the
	init() function for each implementation of these interfaces.

	Note that unlike go-wire, you can call RegisterImplementation separately
	from different locations with each implementation, not all in one place.
...
/*
	...
	kind is the type string used in the json representation, while b is the type
	byte used in the go-wire representation. data is one instance of this concrete
	type, like Bar{}
*/
